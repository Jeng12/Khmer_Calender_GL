...
- Remote API: `CalendarApiRepository` uses HTTPS `HttpURLConnection`, sends Calendar API bearer tokens from `/auth/login` and `/auth/register`, validates JSON responses, and falls back gracefully when API calls fail.
...
- API auth: wired normal login/register to the Calendar API auth endpoints and reject old local-only sessions for Calendar API access.
...
| P0 | Calendar API authentication and per-user client session handling | Implemented app-side; live API smoke test passed |
...
- Fresh launch: splash requires Calendar API login/register before opening synced data screens, then shows database-sync disclosure once.
...
- Live API database checks passed against `https://api-calender-sigma.vercel.app/api/v1`:
  - unauthenticated calendar/data endpoints returned 401
  - `/auth/register` and `/auth/login` returned API bearer tokens
  - authenticated calendar convert/day and work-schedule reads returned 200
  - note, event, and holiday create/read/delete isolation passed with two disposable users
...
The app-side code is substantially stronger after this pass and currently builds/tests. It is suitable for internal testing. Play Store submission still requires hosted privacy policy, Data Safety declarations, target-audience/content-rating declarations, release signing setup, and account deletion/support policy confirmation for API-backed accounts.
